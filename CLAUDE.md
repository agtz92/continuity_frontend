...
Cinco paletas curadas: `ocre` (default) · `salvia` · `oxido` · `hielo` · `ciruela`. Solo pisan `--accent`, `--accent-hi` y `--accent-lo`; `--signal` y `--closed` **no son personalizables** (son semántica, no gusto).

**Fuente de verdad: `src/design/tokens.json`.** Editas ahí y corres `pnpm tokens`, que reescribe `src/app/tokens.generated.css`. Después sincroniza a mano dos espejos: `src/palette/config.ts` (`PALETTE_SWATCHES`, los swatches del selector) y `backend/core/notifications/schema.py` (`CURRENT_PALETTES`, que valida la mutación).

Las 13 paletas viejas (`default`, `continuuit`, `pink`, `midnight`, `neon`…) siguen siendo entrada válida y el CSS generado las pinta con su equivalente curada (`legacyPalettes` en el JSON, `LEGACY_PALETTE_MAP` en `palette/config.ts`). Se retiran en la ola 8 del rediseño — y **no antes de migrar la app móvil**, que aún las manda.

Detalle histórico: `../docs/paletas-de-colores.md` (describe el sistema anterior a `tokens.json`).

## Tailwind: opacidad sobre colores del tema

**No uses `bg-accent/40`, `text-accent/60`, `bg-surface/50`, etc. con los colores del tema.** En este setup **no se procesan** y la regla se ignora sin dar error: el elemento se ve como si la clase no existiera.

Causa: en `tailwind.config.ts` los colores del tema están declarados como `"var(--accent)"`, y las variables CSS guardan **hex**. Tailwind 3.4 no puede aplicar alfa sobre un hex envuelto en `var()` con la sintaxis `/<n>`.

### Usa el token, no la opacidad

El rediseño añadió tokens reales para todos los casos en los que antes se recurría a `/<n>`. Prefiérelos:

| Antes (no pinta) | Ahora |
|---|---|
| `bg-surface/50` | `bg-surface` · `bg-surface-2` · `bg-surface-3` |
| `bg-bg/40` | `bg-well` |
| `bg-black/70` en un scrim | `bg-scrim` |
| `border-border/80` | `border-border` (alias de `--line-10`) o `border-line-14` … `border-line-34` |
| `bg-accent/15` | `bg-accent-a12` (rampa: a12 a14 a22 a35 a50 a55) |
| `bg-red-500/20` para bloqueado | `bg-signal-a12` (rampa: a02 a03 a04 a09 a10 a12 a16 a50 a55) |
| `text-text-muted/60` | `text-text-3` … `text-text-6` |

La escala completa vive en `src/design/tokens.json` y la emite `pnpm tokens`. **No edites `src/app/tokens.generated.css` a mano.**

Si de verdad necesitas un alfa que no está en la rampa, usa `color-mix()` como arbitrary value:

```tsx
className="hover:bg-[color-mix(in_srgb,var(--accent)_18%,transparent)]"
```

### Cuándo SÍ funciona `color/<n>`

Con colores de la paleta de Tailwind (`bg-red-500/20`), que no pasan por variables CSS. Pero el producto **ya no los usa**: estado, prioridad, categoría y urgencia se resuelven con `--signal` / `--closed` / `--accent` / `--line-*`. Los colores fijos quedan solo en marketing y admin.

### Ojo: un nombre de clase inexistente tampoco da error

`bg-accent-a-22` (con guion de más) o `bg-canvas/70` no fallan el build: simplemente no pintan. Si un fondo "no se ve", comprueba primero que la clase exista en el CSS generado.

...
- **Sección:** `src/components/notes/NoteSectionBlock.tsx` — toggle plegable, guarda al perder foco, **drag & drop** con `@dnd-kit` (handle ⠿; el `DndContext`/`SortableContext` viven en `QuickNotesView`), botones ▲▼ como fallback de teclado, y **toggle vista/edición** (👁/✎).
